<template>
  <header class="w-full sticky top-0 z-[100] shadow-sm">
    <div
      class="w-full xl:h-[76px] bg-white dark:bg-[#091a28] sepia:bg-[#f1e7d0] text-black dark:text-white sepia:text-[#433422] relative">
      <div class="w-full h-0 relative">
        <div class="absolute left-0 z-[1000] flex items-center xl:w-[140px] xl:h-[76px] xl:ml-[184px]">
          <NuxtLink to="/">
            <div class="xl:w-[140px] xl:h-[33px] cursor-pointer flex items-center">
              <img class="w-full h-full" src="/chicken.svg" alt="logo" />
              <span class="xl:text-[18px] font-oPPOSansMFont text-nowrap xl:ml-[4px]">
                今天吃什么呢
              </span>
            </div>
          </NuxtLink>
      ...
      <div class="h-full relative xl:px-[184px]">
        <div ref="NavRef" class="h-full relative xl:mx-[140px]">
          <nav class="hidden xl:block h-full xl:m-[0_85px_0_79px]">
            <ul class="font-oPPOSansMFont flex items-center justify-between h-full">
              <li v-for="(optItem, optIndex) in HEADER_OPTIONS" :key="optIndex"
                class="xl:text-[18px]  h-full flex items-center" @mouseover="handleMouseoverMenu(optItem.subMenus)"
                @mouseout="(e) => handleMouseoutMenu(e, optItem.key)">
                <NuxtLink class="transition-all duration-300 hover:cursor-pointer hover:text-[#3370ff]"
                  :to="optItem.href">
                  {{ optItem.name }}
                </NuxtLink>
              </li>
        ...
        <div ref="SubMenusRef" class="w-full flex items-center justify-center absolute xl:top-[76px] left-0" :class="{
          'xl:h-[60px] bg-white opacity-100 transition-all duration-300':
            isShowSubMenus,
          'h-0 transition-all duration-300': !isShowSubMenus,
        }" @mouseover="handleMouseoverSubmenu" @mouseout="handleMouseoutSubmenu">
          <div v-for="(subMenu, index) in sub_options" :key="index"
        ...
        <div ref="searchBoxRef" class="w-full bg-white absolute top-0 left-0 z-[999]" :class="{
          'xl:h-[566px] transition-all duration-700': isShowSearchBox,
          'h-0 transition-all duration-500': !isShowSearchBox,
        }">
          <div class="w-full flex flex-col items-center justify-center" :class="{
            ...
            <div
              ...
              <input v-model="searchKeywords" type="text" :placeholder="searchPlaceholder"
              ...
              <div class="xl:mt-[38px] w-full font-oPPOSansMFont" :class="{
                block: isShowSearchBox,
                hidden: !isShowSearchBox,
              }">
                <div class="text-[#B3B3B3] xl:text-[12px] xl:leading-[13px] xl:mb-[8px]">
                  大家都在搜
                </div>
      ...
      <div class="w-full h-0 relative">
        <div ref="MallRef" class="absolute right-0 flex items-center xl:mr-[184px] xl:h-[76px] xl:top-[-76px]">
          <div class="shrink-0 cursor-pointer xl:w-[23px] h-full relative">
            <div class="w-full h-full flex items-center absolute left-0 top-0 z-[1000]" :class="{
          ...
          <div
            ...
            <div class="shrink-0 h-full flex items-center justify-center">
              <div class="xl:w-[13px] xl:h-[15px]">
                <svg width="100%" height="100%" viewBox="0 0 13 15" version="1.1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink">
                  <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g transform="translate(-1590.000000, -30.000000)" stroke="#FFFFFF">
